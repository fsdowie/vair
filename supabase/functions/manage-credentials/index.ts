...
async function testLogin(siteUrl: string, username: string, password: string): Promise<{ success: boolean; error?: string; debug?: string[] }> {
  ...
  try {
    ...
    // Use dimensions matching cdtWidth/cdtHeight hidden fields (640x480 defaults)
    const w = hiddenFields['cdtWidth'] || '640';
    const h = hiddenFields['cdtHeight'] || '480';
    form.set('cdt', `1,${w},${h},1`);
